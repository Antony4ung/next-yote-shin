...
export default function Details() {
  ...
  useEffect(() => {
    fetchVideoParams(tvId,'tv',setVideoParams);
    fetchDetails(tvId,'tv',setDetailData);
  ...
  return (
    <Box sx={{ width: "100%", height: "100%" }}>
      {modalOpen && <ModalCom params={videoParams} />}
      <Box
        sx={{
          width: "100%",
          minHeight: "60vh",
      ...
      {tvId && <CreditsScroller type='tv' id={tvId} />}
     
      {videoParams && <VideoTrailerRow videoParams={videoParams} />}
      {tvId && <Recommendations setDetailData={setDetailData}  id={tvId} type="tv"/>}
    </Box>
  );
}
